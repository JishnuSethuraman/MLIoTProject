...
# Load the dataset
file_path = 'combined_mapping/sensor_combined.pkl'
...
def create_inout_sequences(input_data, sequence_length):
    inout_seq = []
    L = len(input_data)
    for i in tqdm(range(L - sequence_length),desc="In Out Seq"):
...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_layer_size=100, output_size=9):
        super().__init__()
...
model = LSTMModel(input_size=9).to(device)
...
for epoch in tqdm(range(epochs),desc="epochs"):
    for seq, labels in tqdm(train_loader,desc="epoch"):
        optimizer.zero_grad()
